    ...
    fun createScheduleForDay(scheduleDate: LocalDate) {
        ...
        for (theater in theaters) {
            ...
            val selectedMovies = if (theater.id in 1..2) { // 1관 2관
                top10.firstOrNull()?.let { listOf(it) } ?: continue // ?.let 을 통해 null이 아닌 경우에만 함수를 호출하고 listOf(it)을 통해 리스트를 생성
            } else { // 나머지 영화 중에서 랜덤으로 3~4개 선택
                if (top10.size > 1) top10.drop(1).shuffled().take((3..4).random())
                // 리스트 크기가 1보다 클 경우 첫 번째 영화 삭제 후 2~3개를 랜덤으로 선택
                else continue
                // 영화가 1개 이하라면 다음 반복으로
            }
            ...
            // 8, 9, 10시 시작 시간 랜덤 설정
            while (startTime.plusHours(2) < LocalTime.of(22, 0)) { // 22시 전까지 계속 반복
                val movie = selectedMovies.shuffled().find {
                    scheduleService.movieCompareDate(it.id, scheduleDate) // 무비랑 현재 날짜 비교
                }
                if (movie != null) {
                    val schedule = Schedule().apply {
                        this.start = startTime.toString()
                        this.end = startTime.plusHours(2).toString() // 일단 2시간으로 고정
                    ...
                    if (startTime.plusHours(2) >= LocalTime.of(22, 0)) break
                }
            }
        }
    }
